...
for stat in data["Statistics"]:
    stats.objects.create(university=uni, name=stat["name"], value=stat["value"])

# Step 4: Update Video Links
# Clear existing video links
uni.video_links.all().delete()
for video in data["Video Links"]:
    videos_links.objects.create(university=uni, url=video["url"])

# Step 5: Update FAQs
# Clear existing FAQs
uni.faqs.all().delete()
for faq in data["FAQs"]:
    faqs.objects.create(university=uni, question=faq["question"], answer=faq["answer"])
...
for ranking in data["University Rankings"]:
    ...
    if (
        agency_id not in latest_rankings
        or ranking["year"] > latest_rankings[agency_id]["year"]
    ):
...
for ranking in latest_rankings.values():
    try:
        ...
        university_ranking.objects.create(
            university=uni, ranking_agency=agency, rank=ranking["rank"]
        )
    except ObjectDoesNotExist:
        print(
            f"Ranking agency with ID {ranking['ranking_agency_id']} not found, skipping ranking."
        )
...
for admission in data["Admission Stats"]:
    AdmissionStats.objects.create(
        ...
        GPA_min=int(
            admission["GPA_min"]
        ),  # Convert to int to match PositiveIntegerField
        GPA_max=int(
            admission["GPA_max"]
        ),  # Convert to int to match PositiveIntegerField
...
for work in data["Work Opportunities"]:
    WorkOpportunity.objects.create(university=uni, name=work["name"])
