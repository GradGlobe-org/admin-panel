...
class FAQUpdateInputSchema:
    add: Optional[list[FAQInputSchema]] = None
    update: Optional[list[FAQUpdateSchema]] = None
    delete: Optional[list[int]] = None
...
class ScholarshipSchema(SchemaMixin):
    ...
    def add_scholarship(
        ...
        if not emp.is_superuser:
            if not emp.job_roles.filter(
                    permissions__name="scholarship_add"
            ).exists():
                raise GraphQLError("You do not have permission to add scholarships")

        if not scholarship:
            raise GraphQLError("Scholarship data is required")
        ...
        try:
            with transaction.atomic():
                ...
                unis = list(
                    university.objects.filter(
                        id__in=scholarship.university_ids or []
                    )
                )
                elig_nat = list(
                    Country.objects.filter(
                        id__in=scholarship.eligible_nationalities_ids or []
                    )
                )
                ...
                if unis:
                    sch.university.add(*unis)
                if elig_nat:
                    sch.eligible_nationalities.add(*elig_nat)

                if expenses:
                    expense_ids = [e.expense_type_id for e in expenses]
                    expense_map = {
                        e.id: e
                        for e in ExpenseType.objects.filter(id__in=expense_ids)
                    }

                    if len(expense_map) != len(set(expense_ids)):
                        raise GraphQLError("One or more expense types are invalid")
                    expense_objects = []

                    for exp in expenses:
                        expense_type = expense_map.get(exp.expense_type_id)
                    ...
                    ScholarshipExpenseCoverage.objects.bulk_create(
                        expense_objects
                    )
            ...
            raise GraphQLError("Failed to create scholarship. Please try again")
